...
export const YTPlayer = ((props) => {
  ...
  return (
    ...
    <ReactPlayer style={{displasy: 'none'}} url={`https://music.youtube.com/watch?v=${TrackData.YTid}`} height={opts.height} width={opts.width} playing={isPlaying} controls={false} onBuffer={setIsBuffering(true)} onBufferEnd={setIsBuffering(false)} onDuration={(e) => setMusicDuration(e)} muted={false} onProgress={(e) => {setMusicCurrentTime(e.playedSeconds); setBufferDuration(e.loadedSeconds);}} />
  )
})
